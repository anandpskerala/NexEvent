...
export class UserController {
    constructor(
        @inject("IUserService") public _userService: IUserService, 
        @inject("UserProducer") private _producer: UserProducer<UserDTO>
    ) {}

    public getUser = async (req: Request, res: Response): Promise<void> => {
        const id = req.params.id;
        const result = await this._userService.getUserDetails(id);
        res.status(result.status).json({message: result.message, user: result.user});
    }

    public verifyUser = async (req: Request, res: Response): Promise<void> => {
        const userId = req.headers['x-user-id'] as string;
        const result = await this._userService.verifyUser(userId);
        res.status(result.status).json({ user: result.user });
    }

    public getAllUsers = async (req: Request, res: Response): Promise<void> => {
        const { query = "", page = 1, limit = 10, role = "", status = "", myId } = req.query;
        const result = await this._userService.getAllUsers(query as string, page as number, limit as number, role as string, status as string, myId as string);
    ...
    public updateProfile = async (req: Request, res: Response): Promise<void> => {
        ...
        const result = await this._userService.updateProfile(email, firstName, lastName, phoneNumber);
        res.status(result.status).json({message: result.message, user: result.user});
    }
    ...
    public updateProfileImage = async (req: Request, res: Response): Promise<void> => {
        ...
        const result = await this._userService.updateProfileImage(userId as string, image);
        res.status(result.status).json({message: result.message, user: result.user});
    }

    public updateUser = async (req: Request, res: Response): Promise<void> => {
        const { email, firstName, lastName, phoneNumber, roles, isBlocked } = req.body;
        const result = await this._userService.updateUser(email, firstName, lastName, phoneNumber, roles, isBlocked);
    ...
    public deleteUser = async (req: Request, res: Response): Promise<void> => {
        ...
        if (result.status === StatusCode.OK && result.user) {
            this._producer.sendData(TOPICS.USER_DELETED, result.user)
        }
        res.status(result.status).json({message: result.message});
    }
    ...
    public getBulkUsers = async (req: Request, res: Response): Promise<void> => {
        const { ids } = req.body;
        const result = await this._userService.getUsersBulk(ids);
