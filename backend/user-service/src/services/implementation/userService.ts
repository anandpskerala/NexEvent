...
export class UserService implements IUserService {
    ...
    public async deleteUser(userId: string): Promise<UserReturnType> {
        const user = await this._userRepo.findByID(userId);
        if (!user) {
            return {
                message: HttpResponse.USER_NOT_FOUND,
    ...
    public async getUsersBulk(userIds: string[]): Promise<UsersReturnType> {
        ...
        const users = await this._userRepo.getBulkUsers(userIds);
        return {
            message: HttpResponse.USER_FETCHED,
            status: StatusCode.OK,
